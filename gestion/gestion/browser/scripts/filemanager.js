...
//Return PDF viewer
    var getPDFViewer = function(data) {
	var code = '<div id="pdfviewer"><object data="' + data['Path'] + '#view=Fit" type="application/pdf" width='
		+ config.pdfs.pdfsViewerWidth +' height='
		+ config.pdfs.pdfsViewerHeight +'><p>'
		+ lg.error_pdf_plugin + '</p></object></div>';

	$("#fileinfo img").remove();
	$('#fileinfo #toolbar').before(code);
    };
...
// clicked in the file tree or list views.
	...
	var template = '<div id="preview"><h1></h1><dl></dl><img /></div>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data) {
		...
		if (isPDFFile(data['Filename']) && config.pdfs.showPDFViewer == true) {
		    getPDFViewer(data);
		}
