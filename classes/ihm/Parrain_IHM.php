...
class Parrain_IHM {
  ...
  */
  public static function afficherListeParrainage($tabConventions, $tabContrats) {
    if (sizeof($tabConventions) > 0 || sizeof($tabContrats) > 0) {
      ?>
      <br/>
      <table width="100%">
        <tr id="entete">
          ...
          <td align="center" width="15%">Sujet</td>
        </tr>
        <?php
        ...
        for ($i = 0; $i < sizeof($tabContrats); $i++) {
          ...
          $annee = Promotion_BDD::getLastAnnee();
          $etudiant = $tabContrats[$i]->getEtudiant();
          $promotion = $etudiant->getPromotion($annee);
          $filiere = $promotion->getFiliere();
          $parcours = $promotion->getParcours();
          ?>
          <tr id="ligne<?php echo $cpt%2; $cpt++; ?>">
            ...
            <td>
              <?php
              $parrain = $tabContrats[$i]->getParrain();
              echo $parrain->getNom()." ".$parrain->getPrenom();
              ?>
            </td>
            <td><?php echo $etudiant->getNom()." ".$etudiant->getPrenom(); ?></td>
            ...
            <td>
              <?php
              $entreprise = $tabContrats[$i]->getEntreprise();
              echo $entreprise->getNom()."<br/>".$entreprise->getAdresse()."<br/>".$entreprise->getCodePostal()." ".$entreprise->getVille();
              ?>
            </td>

            <td align="center">
              <a href="./ficheDeContrat.php?annee=<?php echo $_POST['annee']; ?>&parcours=<?php echo $_POST['parcours']; ?>&filiere=<?php echo $_POST['filiere']; ?>&nom=<?php echo $_POST['nom']; ?>&idEtu=<?php echo $etudiant->getIdentifiantBDD(); ?>&idPromo=<?php echo $promotion->getIdentifiantBDD(); ?>"  target="_blank">
                <img src="../images/resume.png" />
              </a>
            </td>
          </tr>
  ...
  */
  public static function afficherListeBilanParrains($tabPromotions, $tabConventions, $tabContrats) {
    if ( (sizeof($tabConventions) > 0 && sizeof($tabPromotions) > 0) || (sizeof($tabContrats) > 0 && sizeof($tabPromotions) > 0) ) {
      ?>
