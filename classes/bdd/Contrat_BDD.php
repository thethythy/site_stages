...
/**
 * Représentation et accès à la table n°4 : les contrats d'alternance
 */
class Contrat_BDD {
	...
	if ($contrat->getIdentifiantBDD() == "") {
	    // Création du Contrat
	    $requete = "INSERT INTO $tab31(idparrain, idexaminateur, idetudiant, idreferent, sujetcontrat, typedecontrat, duree, indemnite, idTheme)
	    ...
	    $requete = "UPDATE $tab31 SET
			idparrain = '" . $parrain->getIdentifiantBDD() . "',
     ...
     * @global resource $db Référence à la base ouverte
     * @global string $tab32 Nom de la table 'relation_promotion_etudiant_contrat'
     * @param objet Contrat $cont Une contrat pour un étudiant
     * @param integer $annee L'année de la contrat
     * @return boolean
     */
    public static function existe($cont, $annee) {
	global $db;
	global $tab32;

	$etu = $cont->getEtudiant();
