...
class Contact {
	...
	public function Contact($identifiantBDD, $nom, $prenom, $telephone, $telecopie, $email, $identifiant_entreprise) {
	...
	public static function getContact($idContact){
		...
		return new Contact($contactBDD["idcontact"], $contactBDD["nomcontact"], $contactBDD["prenomcontact"], $contactBDD["telephone"], $contactBDD["telecopie"], $contactBDD["email"], $contactBDD["identreprise"]);
	}
	...
	public static function getListeContacts($filtres){
		...
		for($i=0; $i<sizeof($tabContactString); $i++)
  			array_push($tabContacts, new Contact($tabContactString[$i][0], $tabContactString[$i][1], $tabContactString[$i][2], $tabContactString[$i][3], $tabContactString[$i][4], $tabContactString[$i][5], $tabContactString[$i][6]));
	...
	public static function listerContacts($identifiant_entreprise){
		...
		for($i=0; $i<sizeof($tabContactsStr); $i++)	
			array_push($tab_contacts, new Contact($tabContactsStr[$i][0],$tabContactsStr[$i][1],$tabContactsStr[$i][2],$tabContactsStr[$i][3],$tabContactsStr[$i][4],$tabContactsStr[$i][5],$tabContactsStr[$i][6]));
