...
class Parrain {
	...
	public static function getParrain($idParrain){
		...
		return new Parrain($parrainBDD["idparrain"], $parrainBDD["nomparrain"], $parrainBDD["prenomparrain"], $parrainBDD["emailparrain"], $parrainBDD["idcouleur"]);
	}
	
	public static function saisirDonneesParrain($tab_donnees){
		$parrain = new Parrain('',$tab_donnees[0],$tab_donnees[1],$tab_donnees[2],$tab_donnees[3]);
	...
	public static function listerParrain(){
		...
		for($i=0; $i<sizeof($tabParrainString); $i++)			
	  		array_push($tabParrain, new Parrain($tabParrainString[$i][0],$tabParrainString[$i][1],$tabParrainString[$i][2],$tabParrainString[$i][3],$tabParrainString[$i][4]));
