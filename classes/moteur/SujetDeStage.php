...
class SujetDeStage {
	...
	public function SujetDeStage($identifiantBDD, $identifiantEtudiant, $identifiantPromotion, $description, $valide, $enAttenteDeValidation) {
	...
	public static function saisirDonnees($tab_donnees){
		$sds = new SujetDeStage("",$tab_donnees[0],$tab_donnees[1],$tab_donnees[2],false,true);
	...
	public static function getSujetDeStageAValider(){	
		...
		for ($i=0; $i<sizeof($tabSdS); $i++)
  			array_push($tabSujetDeStage, new SujetDeStage($tabSdS[$i][0], $tabSdS[$i][1], $tabSdS[$i][2], $tabSdS[$i][3], $tabSdS[$i][4], $tabSdS[$i][5]));
	...
	public static function getSujetDeStage($identifiant){
		...
		return new SujetDeStage($sds["idsujetdestage"], $sds["idetudiant"], $sds["idpromotion"], $sds["description"], $sds["valide"], $sds["enattente"]);
	}
	
	public static function rechercheSujetDeStage($etudiant, $promotion) {
		return SujetDeStage::getSujetDeStage(SujetDeStage_BDD::rechercheSujetDeStage($etudiant->getIdentifiantBDD(), $promotion->getIdentifiantBDD()));
	}
