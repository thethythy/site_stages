...
/**
* Page demanderValidationSDA.php
* Utilisation : page de demande de validation d'un sujet d'alternance
* Dépendance(s) : demanderValidationSDAData.php --> traitement des requêtes Ajax
...
function depotSujet($identifiant) {
  ...
  if ($file && ($extension == "pdf" || $extension == "doc" || $extension == "odt" || $extension == "docx" || $extension == "txt")) {
    ...
    $store_dir = "../documents/sujetsDAlternance/";
...
if (isset($_POST['idetudiant']) && $_POST['idetudiant'] != -1) {
  ...
  if (isset($_FILES['uploadSujet']['name']) && $_FILES['uploadSujet']['name'] != "") { //si un fichier est envoyé
    ...
    if ($filename != "") {
      ...
      SujetDAlternance::saisirDonnees($tabDonnees);
      envoyerNotification();
...
if (!$traitement) {
  ...
  Promotion_IHM::afficherFormulaireRecherche("demanderValidationSDAData.php", false, true);
