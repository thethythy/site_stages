...
function envoyerNotification($oEtudiant, $annee, $idFiliere, $idParcours, $idParrain, $nomFichier, $typedocument) {
  ...
  $oContrat = Contrat::getContratFromEtudiantAndPromotion($oEtudiant->getIdentifiantBDD(), $oPromotion->getIdentifiantBDD());

  $oResponsable = Responsable::getResponsableFromResponsabilite("alternance");
  ...
  Entreprise : " . $oContrat->getContact()->getEntreprise()->getNom() . "<br/>
...
if (isset($_POST['submitRapport']) && $_POST['idEtudiant'] != -1) {
  if (isset($_FILES['uploadRapport']['name']) && $_FILES['uploadRapport']['name'] != "") { //si un fichier est envoyé
    ...
    if ($filename != "") {
      ...
      $oContrat = Contrat::getContratFromEtudiantAndPromotion($_POST['idEtudiant'], $oPromotion->getIdentifiantBDD());
      $idParrain = $oContrat->getIdParrain();
      $chemin = "rapports/" . $filename;
      envoyerNotification($etudiant, $_POST['annee'], $_POST['filiere'], $_POST['parcours'], $idParrain, $chemin, "rapport d'alternance");
      echo "<p>Votre rapport d'alternance a été enregistré et votre référent a été informé de ce dépôt.</p>";
    }
  } else {
...
if (isset($_POST['submitResume']) && $_POST['idEtudiant'] != -1) {
  if (isset($_FILES['uploadResume']['name']) && $_FILES['uploadResume']['name'] != "") { //si un fichier est envoyé
    ...
    if ($filename != "") {
      ...
      envoyerNotification($etudiant, $_POST['annee'], $_POST['filiere'], $_POST['parcours'], $idParrain, $chemin, "résumé d'alternance");
      echo "<p>Votre résumé d'alternance a été enregistré et votre référent a été informé de ce dépôt.</p>";
    }
  } else {
    IHM_Generale::erreur("Vous devez spécifier un fichier !");
  }
